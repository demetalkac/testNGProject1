...
//     Accept alert
// alert'i switchTo yaptık ama calıstırınca "UnhandledAlertException" exception'i verdi, cunku application da "bug" var, bunu cozmek icin try-catch yaptık

            try {
                Driver.getDriver().switchTo().alert().accept();
            }catch (Exception e){   // }catch (Exception e){} yaptıktan sonra hocanın "catch" kısmı daha koyu renkli olunca,uzerinde bekleyince "ignore" yapmamız gerektigi cıktı: "}catch (Exception ignored)" seklinde

            }

//    Add 4 more customers ; 4 kere eklemek icin yazdıklarımızı for icine koyduk
...
//        Select customer name
        ...
        for (int i=6; i<11; i++){
//        Click on "Currency" dropdown
            customerDD.selectByIndex(i); //6'dan baslayacak cunku application da default olarak dropdownda 5 tane kayıtlı var.
//        Select "Dollar"
...
//        Accept alert
            try {
                Driver.getDriver().switchTo().alert().accept(); //"UnhandleAlertException" seluniumdan geliyor.
                                                                // Bu exceptionu handle etmek icin try-catch kullandık.
...
//        xyzBankPage.withdrawSubmitButton.click();  normal click calısmayınca "action classı ile yaptık. sentKey(Key.ENTER) ile veya js ile de yapılabilir.
